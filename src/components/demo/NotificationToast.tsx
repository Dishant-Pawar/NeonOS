...
export const NotificationToast = () => {
  ...
  const getNotificationStyles = (type: string) => {
    switch (type) {
      ...
      case 'warning':
        return 'border-yellow-200 bg-yellow-50/95';
      case 'success':
  ...
  const visibleNotificationData = notifications.filter(notification => 
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 space-y-3 max-w-sm">
      {visibleNotificationData.map((notification, index) => (
        <div
          key={notification.id}
          className={`${getNotificationStyles(notification.type)} backdrop-blur-md rounded-lg shadow-xl border p-4 animate-slide-in-right`}
          style={{
